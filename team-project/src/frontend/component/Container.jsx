...
function Container(props) {
    ...
    const answerBoxB = [
        ...
        {
            mbti: 'N',
            answer: '왜 저런 뜬금없는 게 걸려있지? 사장님 아들이 그리신건가? 아닌가? 저기에 왜 있지? 샀나? 돈 주고 산다고? (이하 생략) ',
        },
    ...
    const handleAnswerA = () => {
        if (count < 8) setIsClick(true);
        setTimeout(() => {
            setArray((prevArray) => [
                { [answerBoxA[count - 1].mbti]: answerA },
                ...prevArray,
            ]);
            if (count === 8) {
                // setAnswerA(answerBoxA[6].answer);
                // setAnswerB(answerBoxB[6].answer);
    ...
    const handleAnswerB = () => {
        ...
        setTimeout(() => {
            setArray((prevArray) => [
                { [answerBoxB[count - 1].mbti]: answerB },
                ...prevArray,
        ...
        <div>
            <StyledContainer className="container">
                <Wrapper className="wrapper">
                    ...
                    <Grid className={`grid ${isClick ? 'click' : ''}`}>
                        <Box
